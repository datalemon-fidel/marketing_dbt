...
WITH filtered_hubspot_leads AS (
  -- Filter for rows where Source_Traffic contains "Google" and excludes "Organic"
  SELECT *
...
date_scaffold AS (
  ...
  FULL OUTER JOIN 
    `rare-guide-433209-e6.AdAccounts.Google Ads` AS fa
  ON 
    hl.Date = fa.Date
),
...
google_ads_aggregated AS (
  -- Aggregate Google Ads Cost to avoid duplication
  SELECT 
...
base_data AS (
  ...
  LEFT JOIN
    google_ads_aggregated AS fa
  ON
    fa.Aggregation_Date = ad.Aggregation_Date  -- Pre-aggregated Ads join
  GROUP BY
    ad.Aggregation_Date, fa.GoogleAds_Cost
),

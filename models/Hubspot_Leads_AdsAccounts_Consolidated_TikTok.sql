...
WITH filtered_hubspot_leads AS (
  -- Filter for rows where Source_Traffic contains "TikTok" and excludes "Organic"
  SELECT *
...
date_scaffold AS (
  ...
  FULL OUTER JOIN 
    `rare-guide-433209-e6.AdAccounts.Tiktok Ads` AS fa
  ON 
    hl.Date = fa.Date
),
...
tiktok_ads_aggregated AS (
  -- Aggregate Tiktok Ads Cost to avoid duplication
  SELECT 
...
base_data AS (
  ...
  GROUP BY
    ad.Aggregation_Date, fa.TikTokAds_Cost
),
